...
function vllm.parse(chunk, assistant_output)
...
function vllm.request(opts)
  ...
  if opts.fim then
    ...
    body["prompt"] = "<|fim_prefix|>\n" .. opts.prompt .. "<|fim_suffix|>\n" .. opts.suffix .. "<|fim_middle|>"
  end
  ...
  for i = 1, opts.n_completions do
    ...
    local new_job = job:new({
      ...
      on_stdout = vim.schedule_wrap(function(_, data)
        ...
        if success then
          assistant_output = vllm.parse(result, assistant_output)
        else
